...
import { AuthUser } from '../auth/decorators/current-auth.decorator';

@Injectable()
export class PostService {
  constructor(private readonly prisma: PrismaService) {}
    
  async create(createPostDto: CreatePostDto, auth: AuthUser) {
    const user = await this.prisma.user.findUnique({ where: { userId: auth.userId }});
    if (!user) throw new NotFoundException(AUTH_MESSAGES.ERROR.NOT_FOUND_USER);

    let createdPost = await this.prisma.post.create({ data: {
      rootPostId: createPostDto.rootPostId,
      parentPostId: createPostDto.parentPostId,
      state: createPostDto.state,
      content: createPostDto.content,
      like: createPostDto.like,
      bookmark: createPostDto.bookmark,
      userId: auth.userId,
    } });
